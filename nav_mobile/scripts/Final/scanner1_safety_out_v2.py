...
class SICK_safety_out():
    ...
    def __init__(self):
        ...
        # define subscriber
        self.sick_sub = rospy.Subscriber("/scanner1/sick_safetyscanners/scan", LaserScan, self.sick_callback)
        # define publisher
        self.safe_out_pub = rospy.Publisher("/scanner1_data", multi_bool, queue_size=10)
    ...
    def sick_callback(self, data):
        ...
        for ii in range(1047):
            ...
            if (laser_measurement) < 0.5:
                self.intrusion_msg.front_danger_thin = True
                # print('zona 1')
                # return

            # field two
            if (laser_measurement) < 1:
                self.intrusion_msg.front_danger_wide = True
                # print('zona 2')
                
            # field three
            if (laser_measurement) < 2:
                self.intrusion_msg.front_alert = True
                # print('zona 3')
...
if __name__ == '__main__':
    # initialise node
    rospy.init_node('sick_safety_fields_s1', anonymous=True)
