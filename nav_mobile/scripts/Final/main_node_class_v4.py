...
class greenbot_safe_control():
    def __init__(self):
        ...
        self.points_sub = rospy.Subscriber("/camera/depth/color/points",PointCloud2, self.pointcloud_callback)
         # TODO FIX NAMES
    ...
    def pointcloud_callback(self, data):
        self.cloud_data = list(pc2.read_points(data, field_names = ("x", "y", "z","rgb"), skip_nans=True))
    ...
    def pointcloud_object(self, viz=False, debug=False):
        kot=23.5
        kot = np.radians(kot)
        ...
        if viz:
            mesh = open3d.geometry.TriangleMesh.create_coordinate_frame(size=0.3, origin=np.array([0.0, 0.0, 0.0]))
            open3d.visualization.draw_geometries([downpcd, mesh]) ##############
            open3d.visualization.draw_geometries([new_cloud, mesh]) ##############a
